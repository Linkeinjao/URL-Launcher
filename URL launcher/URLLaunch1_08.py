...
class WebLauncherApp:
    def __init__(self, master):
        ...
        self.launch_button.grid(row=3, column=1, sticky=tk.W+tk.E)

        self.remove_button = tk.Button(master, text="Remove", command=self.remove_url)
        self.remove_button.grid(row=3, column=2, sticky=tk.W+tk.E)

        self.view_log_button = tk.Button(master, text="View Log", command=self.open_log_file)
        self.view_log_button.grid(row=3, column=3, sticky=tk.W+tk.E)

        # Load image and scale down
        img = tk.PhotoImage(file="Nethergear.png")
        img = img.subsample(10)  # Scale down the image

        # Add image label spanning rows 4 to 7
        img_label = tk.Label(master, image=img)
        img_label.image = img
        img_label.grid(row=4, column=0, rowspan=4, padx=5, pady=0, sticky="w")

        # Add text labels
        text_label = tk.Label(master, text="Nethergear Web launcher Application Version 1.07")
        ...
        if selected_index:
            ...
            for entry in data:
                if entry["name"] == selected_name:
                    ...
                    self.update_log(entry['name'], entry['url'])
                    break
        else:
    ...
    def remove_url(self):
        ...
        if selected_index:
            ...
            selected_name = self.listbox_saved_urls.get(selected_index)
            updated_data = [entry for entry in data if entry["name"] != selected_name]

            with open('urls.json', 'w') as f:
                json.dump(updated_data, f, indent=4)

            self.listbox_saved_urls.delete(selected_index)
            print(f"Removed '{selected_name}'")
        else:
            print("Please select a saved URL.")

    def update_log(self, name, url):
        current_datetime = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_entry = f"{current_datetime}: Launched '{name}' at '{url}'\n"
        try:
    ...
    def open_log_file(self):
        try:
            ...
            messagebox.showwarning("Log File Not Found", "Log file not found.")
